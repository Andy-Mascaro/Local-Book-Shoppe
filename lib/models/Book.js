...
class Book {
  ...
  static async getById(id) {
    const { rows } = await pool.query(
      `SELECT books.*, authors.*, COALESCE(json_agg(to_jsonb(authors))FILTER (WHERE authors.id IS NOT NULL), '[]') as authors FROM books LEFT JOIN works on books.id = works.book_id 
